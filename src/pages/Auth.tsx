      ...
      if (isSignup) {
        ...
        if (userType === "client" && referrerEmail.trim()) {
          ...
          if (!validationResult.valid) {
            if (validationResult.error_code === 'NOT_FOUND') {
              toast.error("O e-mail de indicação não está cadastrado no sistema.");
            } else if (validationResult.error_code === 'NOT_WORKER') {
              toast.error("O e-mail de indicação não pertence a um trabalhador.");
            } else {
              toast.error("E-mail de indicação inválido.");
            }
            setLoading(false);
            return;
          }
              ...
              >
                <div className="text-center">
                  ...
                  <p className="text-muted-foreground text-sm leading-relaxed">
                    Enviamos um código para <span className="text-foreground font-bold">{email}</span>. Insira abaixo para activar sua conta.
                  </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {isSignup && userType === "client" && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="space-y-6 pt-4">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Email de referência (Opcional)</label>
                        <div className="relative">
                          ...
                          <input
                            ...
                            className="input-premium pl-11"
                            placeholder="email@do.trabalhador"
                          />
                        </div>
                        {referrerEmail && (
                          <p className="text-[10px] text-muted-foreground ml-1">
                            Se alguém te recomendou a plataforma, coloca o email dessa pessoa aqui.
                          </p>
                        )}
                      </div>
                    </motion.div>
