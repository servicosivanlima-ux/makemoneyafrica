...
const DashboardHeader = ({ userName, subtitle, userId, accountType }: DashboardHeaderProps) => {
      ...
      <div className="flex items-center justify-between px-6 py-4 lg:pl-6 pl-16">
        <div>
          <div className="flex items-center gap-3">
            ...
            {accountType && (
              <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${accountType === "company"
                ? "bg-primary/10 border-primary/20 text-primary"
                : "bg-gold/10 border-gold/20 text-gold"
                }`}>
                {accountType === "company" ? "Empresa" : "Pessoal"}
              </span>
            )}
          </div>
