...
const WorkerChat = ({ user, profile }: WorkerChatProps) => {
    ...
    const banUser = async (userId: string, userName: string) => {
        ...
        try {
            ...
            toast.success(`Utilizador ${userName} banido temporariamente.`);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 h-[600px]">
                        ...
                        >
                            <div className="flex items-center gap-2 mb-1 px-1">
                                <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-1">
                                    {msg.user_id === user?.id ? "Tu" : msg.profiles?.full_name || msg.profiles?.email?.split('@')[0] || "Utilizador"}
                                    {isAdmin && msg.user_id !== user?.id && (
                                        <button
                                            ...
                                            title="Banir Utilizador Temporariamente"
                                        >
                            ...
                            <div
                                className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm font-semibold shadow-lg transition-all ${msg.is_admin
                                    ? "bg-primary/20 border border-primary/40 text-white shadow-primary/10"
                                    : msg.user_id === user?.id
                                        ? "bg-primary text-black rounded-tr-none shadow-neon/20"
                                        : "bg-white/15 text-white rounded-tl-none border border-white/10"
                                    }`}
                            >
                                {msg.content}
                                ...
                                {isAdmin && (
                                    <button
                                        ...
                                        className={`absolute top-0 p-1.5 bg-red-500/20 hover:bg-red-500/40 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-all z-10 shadow-lg ${msg.user_id === user?.id ? "-left-10" : "-right-10"
                                            } md:static md:opacity-100 md:ml-2`}
                    ...
                    <form onSubmit={sendMessage} className="relative flex items-center gap-2">
                        <div className="relative flex-1">
                            <Input
                                ...
                                placeholder={isModerated ? "Chat desactivado para a sua conta" : "Escreva a sua mensagem..."}
            ...
            <Card className="bg-white/5 border-white/10 rounded-3xl p-6 overflow-hidden flex flex-col">
                <div className="flex items-center gap-2 mb-6">
                    ...
                    <h3 className="text-xs font-black text-white uppercase tracking-widest">Utilizadores Online</h3>
