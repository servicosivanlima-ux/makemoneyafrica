...
const WorkerVerification = ({ profile, onComplete }: WorkerVerificationProps) => {
    ...
    const handleWithdrawSubmit = async (e: React.FormEvent) => {
        ...
        if (!identifier || !holderName) {
            toast.error("Preencha os dados de saque");
            return;
        }
        ...
        try {
            ...
            if (existingMethod) {
                await (supabase as any)
                    ...
                    .update({
                        is_blocked: true,
                        blocked_reason: 'Tentativa de fraude: método de saque duplicado'
                    })
                    .eq('user_id', profile.user_id);
            ...
            toast.success("Método de saque cadastrado com sucesso!");
            onComplete();
        } catch (error: any) {
        ...
        return (
            <div className="card-premium-glow p-8 text-center space-y-6">
                ...
                <div className="bg-white/5 p-4 rounded-xl text-xs text-muted-foreground italic">
                    Você será notificado (a) assim que sua conta for ativada para saques.
                </div>
            </div>
        );
    }
        ...
        <div className="space-y-8">
            ...
            <div className="text-center space-y-2">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary text-[10px] font-black uppercase tracking-widest mb-2">
                    ...
                    Verificação Obrigatória para Iniciar Saques
                </div>
                <h2 className="text-2xl font-black font-display text-white uppercase tracking-widest">Ativação de Conta</h2>
                <p className="text-sm text-muted-foreground">Siga os passos abaixo para habilitar seus saques</p>
            </div>
                            ...
                            {kycStatus === 'rejected' && (
                                <div className="bg-destructive/10 border border-destructive/20 p-4 rounded-xl flex items-start gap-4 mb-4">
                                    ...
                                    <p className="text-xs text-destructive font-medium">
                                        Sua verificação anterior foi rejeitada. Por favor, envie uma foto mais nítida.
                                    </p>
                                </div>
                            )}
                        ...
                        >
                            <div className="bg-gold/10 border border-gold/20 p-4 rounded-xl flex items-start gap-4">
                                ...
                                <p className="text-xs text-gold font-medium leading-relaxed">
                                    Por segurança, o método de saque <b>deve estar no seu nome</b> (titular do documento).
                                </p>
                            </div>
