-- ============================================
-- SQL DE RESTAURAÇÃO COMPLETA
-- Este script cria TUDO (incluindo 'profiles') se não existir.
-- É seguro rodar em qualquer estado da base de dados.
-- ============================================

-- 1. Criação de ENUMs (ignora se já existirem)
DO $$ BEGIN
...
-- 3. Criação da tabela user_roles
...
-- 6. Criação da tabela withdrawals
...
-- 7. Criação da tabela notifications
...
-- 8. Criação da tabela blocked_devices
...
-- 9. Ativar RLS (safe se já estiver ativado)
...
DO $$ BEGIN ALTER TABLE public.withdrawals ENABLE ROW LEVEL SECURITY; EXCEPTION WHEN OTHERS THEN NULL; END $$;
DO $$ BEGIN ALTER TABLE public.notifications ENABLE ROW LEVEL SECURITY; EXCEPTION WHEN OTHERS THEN NULL; END $$;
DO $$ BEGIN ALTER TABLE public.blocked_devices ENABLE ROW LEVEL SECURITY; EXCEPTION WHEN OTHERS THEN NULL; END $$;

-- 10. Funções Auxiliares (OR REPLACE)
...
-- 11. Policies de Segurança (DROP + CREATE para update seguro)
-- Profiles
...
CREATE POLICY "Clients can view own campaigns" ON public.campaigns FOR SELECT USING (auth.uid() = client_id);

DROP POLICY IF EXISTS "Clients can create campaigns" ON public.campaigns;
CREATE POLICY "Clients can create campaigns" ON public.campaigns FOR INSERT WITH CHECK (auth.uid() = client_id);

DROP POLICY IF EXISTS "Workers can view active campaigns" ON public.campaigns;
CREATE POLICY "Workers can view active campaigns" ON public.campaigns FOR SELECT USING (status = 'active' AND public.has_role(auth.uid(), 'worker'));
...
-- Tasks
DROP POLICY IF EXISTS "Workers can view available and own tasks" ON public.tasks;
CREATE POLICY "Workers can view available and own tasks" ON public.tasks FOR SELECT USING (status = 'available' OR worker_id = auth.uid());

DROP POLICY IF EXISTS "Workers can claim tasks" ON public.tasks;
CREATE POLICY "Workers can claim tasks" ON public.tasks FOR UPDATE USING ((status = 'available' AND worker_id IS NULL) OR worker_id = auth.uid());
...
-- Withdrawals
DROP POLICY IF EXISTS "Workers can view own withdrawals" ON public.withdrawals;
CREATE POLICY "Workers can view own withdrawals" ON public.withdrawals FOR SELECT USING (auth.uid() = worker_id);

DROP POLICY IF EXISTS "Workers can create withdrawals" ON public.withdrawals;
CREATE POLICY "Workers can create withdrawals" ON public.withdrawals FOR INSERT WITH CHECK (auth.uid() = worker_id);
...
-- Blocked Devices
DROP POLICY IF EXISTS "Admins can manage blocked devices" ON public.blocked_devices;
CREATE POLICY "Admins can manage blocked devices" ON public.blocked_devices FOR ALL USING (public.has_role(auth.uid(), 'admin'));
...
-- 13. Views e Funcionalidades Avançadas
DROP VIEW IF EXISTS public.available_campaigns_for_workers;
CREATE VIEW public.available_campaigns_for_workers WITH (security_invoker = on) AS
...
DECLARE _user_type text; _role app_role;
BEGIN
  ...
  INSERT INTO public.profiles (user_id, full_name, email, phone, user_type, account_type, company_name, withdrawal_method, withdrawal_details, facebook_link, instagram_link, tiktok_link, youtube_link)
  VALUES (NEW.id, COALESCE(NEW.raw_user_meta_data->>'name', 'Usuário'), NEW.email, COALESCE(NEW.raw_user_meta_data->>'phone', ''), _user_type,
    ...
    NEW.raw_user_meta_data->'social_links'->>'tiktok', NEW.raw_user_meta_data->'social_links'->>'youtube');
...
-- Funções Admin
CREATE OR REPLACE FUNCTION public.admin_approve_campaign(p_campaign_id uuid) RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF NOT has_role(auth.uid(), 'admin'::app_role) THEN RAISE EXCEPTION 'Unauthorized'; END IF;
  UPDATE campaigns SET status = 'active', payment_confirmed_at = now(), updated_at = now() WHERE id = p_campaign_id AND status = 'pending_payment';
...
BEGIN
  IF NOT has_role(auth.uid(), 'admin'::app_role) THEN RAISE EXCEPTION 'Unauthorized'; END IF;
  UPDATE campaigns SET status = 'cancelled', updated_at = now() WHERE id = p_campaign_id AND status = 'pending_payment';
  IF NOT FOUND THEN RAISE EXCEPTION 'Campaign not found'; END IF;
  RETURN true;
END; $$;
...
BEGIN
  IF NOT has_role(auth.uid(), 'admin'::app_role) THEN RAISE EXCEPTION 'Unauthorized'; END IF;
  UPDATE tasks SET status = 'approved', reviewed_at = now(), reviewed_by = auth.uid() WHERE id = p_task_id AND status = 'pending_review' RETURNING campaign_id INTO v_campaign_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'Task not found'; END IF;
  UPDATE campaigns SET completed_count = COALESCE(completed_count, 0) + 1, updated_at = now() WHERE id = v_campaign_id;
  RETURN true;
END; $$;
...
BEGIN
  IF NOT has_role(auth.uid(), 'admin'::app_role) THEN RAISE EXCEPTION 'Unauthorized'; END IF;
  UPDATE tasks SET status = 'rejected', rejection_reason = p_reason, reviewed_at = now(), reviewed_by = auth.uid() WHERE id = p_task_id AND status = 'pending_review';
  IF NOT FOUND THEN RAISE EXCEPTION 'Task not found'; END IF;
  RETURN true;
END; $$;

CREATE OR REPLACE FUNCTION public.admin_approve_withdrawal(p_withdrawal_id uuid) RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF NOT has_role(auth.uid(), 'admin'::app_role) THEN RAISE EXCEPTION 'Unauthorized'; END IF;
  UPDATE withdrawals SET status = 'approved', reviewed_at = now(), reviewed_by = auth.uid() WHERE id = p_withdrawal_id AND status = 'pending';
  IF NOT FOUND THEN RAISE EXCEPTION 'Withdrawal not found'; END IF;
  RETURN true;
END; $$;

CREATE OR REPLACE FUNCTION public.admin_reject_withdrawal(p_withdrawal_id uuid, p_reason text) RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF NOT has_role(auth.uid(), 'admin'::app_role) THEN RAISE EXCEPTION 'Unauthorized'; END IF;
  UPDATE withdrawals SET status = 'rejected', rejection_reason = p_reason, reviewed_at = now(), reviewed_by = auth.uid() WHERE id = p_withdrawal_id AND status = 'pending';
  IF NOT FOUND THEN RAISE EXCEPTION 'Withdrawal not found'; END IF;
  RETURN true;
END; $$;

-- Função worker_claim_task
...
DECLARE v_task_id uuid; v_worker_id uuid; v_campaign campaigns%ROWTYPE; v_reward_amount integer; v_existing_task_count integer;
BEGIN
  ...
  SELECT COUNT(*) INTO v_existing_task_count FROM tasks WHERE campaign_id = p_campaign_id AND worker_id = v_worker_id;
  IF v_existing_task_count > 0 THEN RAISE EXCEPTION 'Você já tem uma tarefa para esta campanha'; END IF;
...
CREATE POLICY "Workers can upload their proofs" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'task-proofs' AND (storage.foldername(name))[1] = auth.uid()::text);

DROP POLICY IF EXISTS "Anyone can view task proofs" ON storage.objects;
CREATE POLICY "Anyone can view task proofs" ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'task-proofs');

DROP POLICY IF EXISTS "Workers can update their proofs" ON storage.objects;
CREATE POLICY "Workers can update their proofs" ON storage.objects FOR UPDATE TO authenticated USING (bucket_id = 'task-proofs' AND (storage.foldername(name))[1] = auth.uid()::text);

DROP POLICY IF EXISTS "Workers can delete their proofs" ON storage.objects;
CREATE POLICY "Workers can delete their proofs" ON storage.objects FOR DELETE TO authenticated USING (bucket_id = 'task-proofs' AND (storage.foldername(name))[1] = auth.uid()::text);

-- Função create_campaign_secure
...
CREATE OR REPLACE FUNCTION public.create_campaign_secure(
  ...
  p_payment_proof_url text, p_profile_link text DEFAULT NULL, p_video_link text DEFAULT NULL
) RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_campaign_id uuid; v_price integer; v_target_count integer; v_user_type text;
BEGIN
  SELECT user_type INTO v_user_type FROM profiles WHERE user_id = auth.uid();
  ...
  ELSE
    ...
    END CASE;
  END IF;
  IF EXISTS (SELECT 1 FROM campaigns WHERE client_id = auth.uid() AND status = 'pending_payment') THEN
    RAISE EXCEPTION 'You already have a pending campaign';
  END IF;
  INSERT INTO campaigns (client_id, plan_type, plan_name, platform, page_link, payment_proof_url, profile_link, video_link, target_count, price, status)
  VALUES (auth.uid(), p_plan_type::plan_type, p_plan_name, p_platform::platform_type, p_page_link, p_payment_proof_url, p_profile_link, p_video_link, v_target_count, v_price, 'pending_payment'::campaign_status)
